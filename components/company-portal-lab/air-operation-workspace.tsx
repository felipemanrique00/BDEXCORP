...
interface AirOperationWorkspaceProps {
  demand: CorporateDemandSnapshot
  company: Empresa
  onCompleted: () => void
}

export function AirOperationWorkspace({
  demand,
  company,
  onCompleted,
}: AirOperationWorkspaceProps) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      setReservations([])
      setError(cause instanceof Error ? cause.message : 'Não foi possível carregar a operação desta demanda.')
    } finally {
  ...
  if (loading) {
    return (
      <div className="bbt-card flex min-h-44 items-center justify-center gap-2 p-6 text-sm text-slate-500" role="status">
        <Loader2 className="h-4 w-4 animate-spin" />Carregando reserva e emissão...
      </div>
    )
  }
  ...
  return (
    <div data-company-portal-air-operation>
