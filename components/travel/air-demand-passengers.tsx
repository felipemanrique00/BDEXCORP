...
import { AlertTriangle, CheckCircle2, Loader2, Search, Star, Trash2, UserRound, UsersRound } from 'lucide-react'
...
type DirectoryTraveler = TravelerDirectoryItem & { profileIssues?: unknown }
...
export function AirDemandPassengers({
  ...
  return (
    <section
      ...
      aria-labelledby="air-demand-passengers-title"
    >
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          ...
          <div>
            <h4 id="air-demand-passengers-title" className="font-semibold text-bbt-primary dark:text-white">
              Passageiros
            </h4>
      ...
      <div className="relative">
        ...
        {open && companyId && !disabled && (
          <div className="absolute z-50 mt-1 max-h-64 w-full overflow-auto rounded-lg border border-slate-200 bg-white p-1 shadow-xl dark:border-slate-700 dark:bg-slate-900">
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex w-full items-start justify-between gap-3 rounded-md px-3 py-2 text-left text-sm hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-45 dark:hover:bg-cyan-950/30"
                >
            ...
            {!loading && !error && items.length === 0 && (
              <div className="px-3 py-5 text-center text-xs text-slate-500">Nenhum viajante ativo encontrado.</div>
            )}
      ...
      {validation.blockingIssues.length > 0 && (
        <div role="alert" className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/20 dark:text-amber-100">
          Complete o cadastro dos passageiros destacados antes de enviar. Dados obrigatórios: CPF, data de nascimento, primeiro e último nome.
        </div>
      )}
