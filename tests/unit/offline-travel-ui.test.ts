...
describe('offline travel UI wiring', () => {
  ...
  it('locks approved hotel, stay and price fields while allowing an operational supplier divergence', () => {
    ...
    expect(offlineForm).toContain('readOnly={locksSelectedCommercialQuote}')
    ...
    expect(offlineForm).toContain("['hotelaria', 'aereo', 'locacao', 'rodoviario'].includes(serviceKey)")
    expect(offlineForm).toContain('requiresSelectedCommercialQuote && !selectedHotelQuote && !selectedAirQuote && !selectedGroundQuote')
  ...
  it('locks approved hotel guests outside the audited correction path and hides technical metadata from common users', () => {
    ...
    expect(offlineForm).toContain('readOnly={locksSelectedPassengers}')
