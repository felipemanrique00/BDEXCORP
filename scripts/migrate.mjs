...
async function grantApplicationPrivileges(client) {
  ...
  const statements = await client.query(
    `select unnest(array[
       format('grant connect on database %I to %I', $1, $2),
       format('grant usage on schema public to %I', $2),
       format('grant select, insert, update, delete on all tables in schema public to %I', $2),
       format('grant usage, select, update on all sequences in schema public to %I', $2),
       format('alter default privileges in schema public grant select, insert, update, delete on tables to %I', $2),
       format('alter default privileges in schema public grant usage, select, update on sequences to %I', $2)
     ]) as sql`,
