...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'travel-cancel:post', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const result = await executeGovernedTravelCancellation(
      guard.principal!,
      {
        ...
        idempotencyKey: body.idempotencyKey || request.headers.get('idempotency-key') || `${guard.requestId}:cancel`,
        policyJustification: body.policyJustification,
        confirmed: body.confirmed === true,
        reason: body.reason,
        payload: body.DadosConsulta || body.dadosConsulta || body.payload || {},
      },
      'cancel',
      integrationRegistry.tech.cancel,
    )
    return NextResponse.json({ ok: true, ...result })
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ ok: false, error: 'Dados invalidos para cancelamento.', details: error.flatten() }, { status: 400 })
    }
