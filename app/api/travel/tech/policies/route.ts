...
export async function GET(request: Request) {
  const guard = await guardApiRequest(request, { requireAuth: true, permission: 'gerenciar_integracoes', roleKeys: ['tenant_admin', 'supervisor'], rateLimit: { key: 'tech-policies:get', limit: 80, windowMs: 60_000 } })
  ...
  try {
    ...
    const data = await integrationRegistry.tech.policies(scope.providerCompanyId)
