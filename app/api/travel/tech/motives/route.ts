...
export async function GET(request: Request) {
  const guard = await guardApiRequest(request, { requireAuth: true, roles: ['master'], rateLimit: { key: 'tech-motives:get', limit: 80, windowMs: 60_000 } })
  if (guard.response) return guard.response
  try {
    const providerCompanyId = new URL(request.url).searchParams.get('providerCompanyId')
    const data = await integrationRegistry.tech.motives(providerCompanyId)
