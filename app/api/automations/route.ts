import { NextResponse } from 'next/server'
import { z } from 'zod'

import { automationStatusSchema } from '@/lib/automations'
import { guardApiRequest, runInApiGuardContext } from '@/lib/security/api-guard'
import { readJsonBodyResult } from '@/lib/security/request-body'
import {
  createAutomationDraft,
  listAutomations,
} from '@/lib/server/automation-service'
import {
  governanceBodyErrorResponse,
  governanceErrorResponse,
} from '@/lib/server/governance-api'

export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'

const querySchema = z.object({
  status: automationStatusSchema.optional(),
  eventType: z.string().trim().min(3).max(120).optional(),
  search: z.string().trim().min(1).max(200).optional(),
  limit: z.coerce.number().int().min(1).max(200).default(50),
  offset: z.coerce.number().int().min(0).default(0),
}).strict()

export async function GET(request: Request) {
  const guard = await guardApiRequest(request, {
    requireAuth: true,
    permission: 'executar_automacoes',
    authorization: {
      resource: 'automations',
      action: 'list',
      requiredPermission: 'executar_automacoes',
      allowEmptyCompanyScope: true,
    },
    rateLimit: { key: 'automations:list', limit: 120, windowMs: 60_000 },
  })
  if (guard.response) return guard.response
  try {
    const input = querySchema.parse(Object.fromEntries(new URL(request.url).searchParams))
    const result = await runInApiGuardContext(
      guard,
      () => listAutomations(guard.principal!, input),
    )
    return NextResponse.json(
      { ok: true, ...result },
      { headers: { 'X-Request-Id': guard.requestId, 'Cache-Control': 'no-store, private' } },
    )
  } catch (error) {
    return governanceErrorResponse(error, guard.requestId)
  }
}

export async function POST(request: Request) {
  const guard = await guardApiRequest(request, {
    requireAuth: true,
    permission: 'gerenciar_automacoes',
    authorization: {
      resource: 'automations',
      action: 'create',
      requiredPermission: 'gerenciar_automacoes',
      allowEmptyCompanyScope: true,
    },
    rateLimit: { key: 'automations:create', limit: 20, windowMs: 60_000 },
  })
  if (guard.response) return guard.response
  const input = await readJsonBodyResult<unknown>(request, 512 * 1024)
  if (!input.ok) return governanceBodyErrorResponse(input, guard.requestId)
  try {
    const automation = await runInApiGuardContext(
      guard,
      () => createAutomationDraft(guard.principal!, input.body),
    )
    return NextResponse.json(
      { ok: true, automation },
      {
        status: 201,
        headers: { 'X-Request-Id': guard.requestId, 'Cache-Control': 'no-store, private' },
      },
    )
  } catch (error) {
    return governanceErrorResponse(error, guard.requestId)
  }
}
