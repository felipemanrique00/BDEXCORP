...
import { createHotelCatalog, listHotelCatalog } from '@/lib/server/hotel-catalog-service'
...
export async function GET(request: Request) {
  ...
  try {
    const result = await listHotelCatalog(
      guard.principal!,
...
export async function POST(request: Request) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'hotel-catalog:create', limit: 40, windowMs: 60_000 },
  })
  if (guard.response) return guard.response
  const input = await readJsonBodyResult<unknown>(request, 512 * 1024)
  if (!input.ok) return governanceBodyErrorResponse(input, guard.requestId)
  try {
    const item = await createHotelCatalog(guard.principal!, input.body)
    return NextResponse.json(
      { ok: true, item },
