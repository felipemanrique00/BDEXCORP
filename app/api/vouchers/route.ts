...
import { createVoucher, listVouchers } from '@/lib/server/voucher-service'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await listVouchers(guard.principal!, query)
...
export async function POST(request: Request) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'vouchers:create', limit: 60, windowMs: 60_000 },
  })
  if (guard.response) return guard.response
  ...
  try {
    const voucher = await createVoucher(guard.principal!, input.body)
    return NextResponse.json(
      { ok: true, voucher },
      {
        status: 201,
