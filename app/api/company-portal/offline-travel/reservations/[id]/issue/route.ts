...
import { issueCompanyPortalOfflineReservation } from '@/lib/server/company-portal-offline-travel-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'company-portal:offline-travel:issue', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const result = await issueCompanyPortalOfflineReservation(guard.principal!, id, input.body)
