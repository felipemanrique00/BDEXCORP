...
import { submitCompanyPortalTravelOrder } from '@/lib/server/company-portal-travel-order-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'company-portal:travel-orders:submit', limit: 30, windowMs: 60_000 },
  })
  ...
  return runInApiGuardContext(guard, async () => {
    ...
    try {
      ...
      const result = await submitCompanyPortalTravelOrder(
