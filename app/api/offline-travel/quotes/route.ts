...
import { createOfflineHotelQuote, listOfflineHotelQuotes } from '@/lib/server/offline-quote-service'
...
export async function GET(request: Request) {
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'offline-travel:quotes:list', limit: 120, windowMs: 60_000 },
  })
  if (guard.response) return guard.response

  try {
    const query = quoteListQuerySchema.parse(Object.fromEntries(new URL(request.url).searchParams))
    const result = await listOfflineHotelQuotes(guard.principal!, query.demandId)
...
export async function POST(request: Request) {
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'offline-travel:quotes:create', limit: 40, windowMs: 60_000 },
  })
  if (guard.response) return guard.response
  ...
  try {
    const created = await createOfflineHotelQuote(guard.principal!, input.body)
    return NextResponse.json(
