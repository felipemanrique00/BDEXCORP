...
import { createOfflineReservation } from '@/lib/server/offline-travel-service'
...
export async function POST(request: Request) {
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'offline-travel:reserve', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    const result = await createOfflineReservation(guard.principal!, input.body)
