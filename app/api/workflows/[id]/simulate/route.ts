...
import { simulateEnterpriseWorkflowDefinition } from '@/lib/server/enterprise-workflow-definition-service'
import {
...
export async function POST(
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'enterprise-workflows:simulate', limit: 60, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const simulation = await runInApiGuardContext(
      guard,
      () => simulateEnterpriseWorkflowDefinition(guard.principal!, id, input.body),
    )
