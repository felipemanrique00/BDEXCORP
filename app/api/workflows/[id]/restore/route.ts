...
import { restoreEnterpriseWorkflowVersion } from '@/lib/server/enterprise-workflow-definition-service'
import {
...
export async function POST(
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'enterprise-workflows:restore', limit: 10, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const workflow = await runInApiGuardContext(
      guard,
      () => restoreEnterpriseWorkflowVersion(guard.principal!, id, input.body),
    )
