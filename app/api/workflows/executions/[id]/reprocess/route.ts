...
import { reprocessEnterpriseWorkflowStep } from '@/lib/server/enterprise-workflow-runtime-service'
...
export async function POST(
  ...
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'enterprise-workflows:step-reprocess', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const execution = await runInApiGuardContext(
      guard,
      () => reprocessEnterpriseWorkflowStep(guard.principal!, id, input.body),
    )
