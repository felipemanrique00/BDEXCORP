...
import { updateDemandAssignment } from '@/lib/server/demand-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'demands:assignment', limit: 60, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const result = await updateDemandAssignment(guard.principal!, id, input.body)
