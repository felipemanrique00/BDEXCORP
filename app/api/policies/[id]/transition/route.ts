...
import { transitionPolicyVersion } from '@/lib/server/policy-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'policies:transition', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    const policy = await transitionPolicyVersion(guard.principal!, id, input.body)
