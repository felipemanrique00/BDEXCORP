...
import { revokeApprovalAuthority } from '@/lib/server/approval-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'approval-authorities:revoke', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    await revokeApprovalAuthority(guard.principal!, id, input.body)
