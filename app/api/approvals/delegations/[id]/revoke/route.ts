...
import { revokeApprovalDelegation } from '@/lib/server/approval-service'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  const guard = await guardApiRequest(request, {
    ...
    rateLimit: { key: 'approval-delegations:revoke', limit: 30, windowMs: 60_000 },
  })
  ...
  try {
    ...
    await revokeApprovalDelegation(guard.principal!, id, input.body)
